...
void loop() {
  ...
  int32_t rollCTRL = pulse2;
  int32_t yawCTRL = pulse4;
  ...
  if (ARMED == true) {
    ...
    Duty = (2 * (ThrottleIN - 1000)) + .25*((pitchCTRL - 1500) * cos(MotorRads)) + .25*((rollCTRL - 1500) * sin(MotorRads));
    Serial.println(Duty);
  } else {
    Duty = 0;
  }
  ...
  // Serial.print("   rollCTRL: "); Serial.println(rollCTRL);
